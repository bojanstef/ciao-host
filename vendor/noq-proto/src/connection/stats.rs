...
pub struct PathStats {
    ...
    pub current_mtu: u16,
    /// Consecutive probe timeouts (RFC 9002 §6.2) on this path with nothing acknowledged in
    /// between; reset to zero by an acknowledgement of this path's packets.
    ///
    /// Ciao patch on noq-proto 1.0.1 (see vendor/noq-proto/CIAO-PATCH.md): the one loss-detection
    /// value that says "this path's sends are not being answered" per path, which neither
    /// `lost_packets` nor `cwnd` can — with no acknowledgement to compare against, nothing is
    /// ever declared lost.
    pub pto_count: u32,
}
