...
/// Represents the master end of a pty.
///
/// CIAO PATCH (see ../CIAO-PATCH.md): upstream sends `\n` followed by the tty's EOF character
/// into the pty when this is dropped, so a shell reading its stdin exits. A client that has put
/// the terminal into raw mode does not read that as end-of-file; it reads a Ctrl-J and a Ctrl-D
/// typed at it, and an attach client (tmux, herdr) forwards both to the pane it is attached to,
/// where a shell then really does exit. Ciao ends children with its own signal ladder and must
/// never type into a terminal it is closing, so the writer's drop here closes and writes nothing.
