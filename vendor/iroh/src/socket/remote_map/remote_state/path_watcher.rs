...
impl<'conn> Path<'conn> {
    ...
    // Ciao patch on iroh 1.0.2 (see vendor/iroh/CIAO-PATCH.md): the two per-path controls
    // noq already offers, reachable from the snapshot an application holds. An application
    // that has decided a selected path is dead — its sends have stopped being acknowledged
    // while another path is open — can abandon it now instead of waiting for the ~15 s
    // path idle timeout, and iroh's own selector then moves to the next path.

    /// Closes this path. Once noq has drained in-flight packets a
    /// [`PathEvent::Abandoned`](crate::endpoint::PathEvent) follows and the selector
    /// re-evaluates the remaining paths.
    pub fn close(&self) -> Result<(), noq::ClosePathError> {
        self.data.upgrade(self.conn).close()
    }

    /// Sets this path's max idle timeout; returns the previous value.
    pub fn set_max_idle_timeout(
        &self,
        timeout: Option<Duration>,
    ) -> Result<Option<Duration>, noq::ClosedPath> {
        self.data.upgrade(self.conn).set_max_idle_timeout(timeout)
    }
}
